...
Commits `97718f44ed`, `72f8318c9c`, and `d9024ddb5c` add bounded diagnostic-only reseparation receipts and remove a duplicate split-migration symbol found by the exact link. Each movement attempt receives a deterministic submission ID and retains exact attempt, wipe, route, map, instance, source, member, candidate, safety, arbitration, active-path, native-motion, progress, arrival, and closure fields. Repeated submissions against one selected anchor remain distinct. Commit `e600346648` widens `experiment_bot_events.context_json` to `MEDIUMTEXT` so the full receipt payload is stored without truncation. These changes do not alter candidate admission, movement arbitration, encounter state, damage, threat, healing, or death recovery.

The next run must build the exact clean commit `d9024ddb5cc9e213e9f0048444b66522cd4d005c`, tree `79dd9b4d2ee29e24074dd0cef3ebcc384065d376`, through the shared queued coordinator. Provision a fresh ten-player Magmaw diagnostic roster and run one completion-watchdog shard. There is no fixed raid success timer. Extract the first per-submission divergence between selection, arbitration, active path, progress, arrival, and closure, then route at most one trace-backed behavior repair. Do not teleport, force movement or threat, revive bots, mutate native encounter state, or manufacture an outcome.
