...
def build_raid_shard_character_sql(config: dict[str, Any], dbc_dir: Path, plan: dict[str, Any],
    ...
    lines = ["-- Generated by tools.raid_program.raid_loadout_sql (raid-shard two-spec loadouts).",
             "-- WARNING: applying this file directly bypasses tools.raid_program.raid_shard_preflight",
             "-- (the reservation-wide allocator check, `pgrep -x worldserver` and the same-server check).",
             "-- Apply it through `raid_shard_preflight --apply --attest-no-worldserver-running`.",
...
def cohort_statements(plan: dict[str, Any], scenario_ids: Sequence[str], gear_profiles: Path,
                      dbc_dir: Path, trainers_path: Path = DEFAULT_TRAINERS) -> list[tuple[str, list[str]]]:
    """(scenario_id, statements) per cohort in the given order, for execute_cohort_transactions."""
    config = prepare_config(plan, gear_profiles, dbc_dir, scenario_ids, trainers_path)
...
def scoped_provisioning_sql(plan_path: Path, scenario_ids: Sequence[str], gear_profiles: Path,
                            dbc_dir: Path, trainers_path: Path = DEFAULT_TRAINERS) -> dict[str, Any]:
...
def plan_payloads(plan: dict[str, Any], gear_profiles: Path, dbc_dir: Path,
                  trainers_path: Path = DEFAULT_TRAINERS) -> dict[str, str]:
    config = prepare_config(plan, gear_profiles, dbc_dir, trainers_path=trainers_path)
    return {
...
def verify_plan_outputs(plan: dict[str, Any], output_dir: Path, gear_profiles: Path,
                        dbc_dir: Path, trainers_path: Path = DEFAULT_TRAINERS) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    """Regenerate a plan's outputs and require the written files and manifest to match."""
    payloads = plan_payloads(plan, gear_profiles, dbc_dir, trainers_path)
    ...
    if manifest.get("materialization_inputs") != inputs:
        failures.append({"check": "raid_shard_manifest_materialization_inputs",
                         "manifest": manifest.get("materialization_inputs"), "actual": inputs})
    return failures, evidence


def write_plan_outputs(plan: dict[str, Any], gear_profiles: Path, dbc_dir: Path,
                       output_dir: Path | None, trainers_path: Path = DEFAULT_TRAINERS) -> dict[str, Any]:
    payloads = plan_payloads(plan, gear_profiles, dbc_dir, trainers_path)
