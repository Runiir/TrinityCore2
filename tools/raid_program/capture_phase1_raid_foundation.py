...
def main() -> int:
    ...
    run_result = execute_capture_run(setup)
    started_utc = run_result.started_utc
    recovery_required = run_result.recovery_required
    stable = run_result.stable
    last_rejections = run_result.last_rejections
    startup_error = run_result.startup_error
    process_return_code = run_result.process_return_code
    telemetry_scheduler = run_result.telemetry_scheduler
    telemetry_transport_ledger = run_result.telemetry_transport_ledger
    telemetry_command_counts = run_result.telemetry_command_counts
    trace_transport_pressure_gate = run_result.trace_transport_pressure_gate
    operator_interrupt = run_result.operator_interrupt
    shutdown_error = run_result.shutdown_error
    stop_commands_sent = run_result.stop_commands_sent
    checkpoint_arm_command_sent = run_result.checkpoint_arm_command_sent
    checkpoint_arm_gate = run_result.checkpoint_arm_gate
    resource_samples = run_result.resource_samples
    resource_sampling_errors = run_result.resource_sampling_errors
    resource_sampling_error_count = run_result.resource_sampling_error_count
    resource_tick_rate = run_result.resource_tick_rate
    forced_evidence_report = run_result.forced_evidence_report
    terminal_failure = run_result.terminal_failure
    semantic_stall = run_result.semantic_stall
    controller_watchdog = run_result.controller_watchdog
    trace_transport_gate = run_result.trace_transport_gate
    telemetry_abort = run_result.telemetry_abort
    log_bytes = run_result.log_bytes
