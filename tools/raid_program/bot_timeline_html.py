...
function render(){{const a=$('actor').value,p=$('phase').value,t=$('target').value,s=$('spell').value,from=lo+Number($('from').value||0)*1000,to=lo+Number($('to').value||span/1000)*1000,targetMatch=e=>!t||(t[0]==='e'&&String(e.target_entry)===t.slice(2))||(t[0]==='g'&&String(e.target_guid)===t.slice(2)),visible=events.filter(e=>e.at_ms>=from&&e.at_ms<=to&&(!a||String(e.actor_guid)===a)&&(!p||String(e.phase)===p)&&targetMatch(e)&&(!s||String(e.spell_id)===s)),pages=Math.max(1,Math.ceil(visible.length/pageSize));page=Math.min(page,pages-1);
