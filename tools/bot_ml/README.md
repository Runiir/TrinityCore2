...
`build_phase9_pairwise_matrix.py` deterministically reconstructs the 24-target pair universe and a complete five-player Stonecore covering array. The pinned seven-canary serial union covers every supported target, retains native Hunter threat-transfer support for every supported tank row, and runs only one exact party at a time. The Python runner selects the cohort, profile, and exact party but never restores character position or manufactures health, power, alive, ghost, or corpse state. While the selected cohort is still inactive, the server population coordinator performs one one-shot entrance provisioning step from the pinned Stonecore route, verifies the resulting native group, heroic difficulty, roster, and entrance state, commits an admission receipt, and only then enables bot actions. Once active, bots and the runner cannot reposition or normalize state; deaths use native combat resurrection or Release Spirit, ghost entrance traversal, and corpse reclaim. The serial operator fails closed unless the server-owned preactivation receipt is exact. `verify_phase9_pairwise_matrix.py` independently reconstructs the roster shape, target exclusions, required/excluded pairs, composition hashes, serial union, representation bounds, and matrix identity.
