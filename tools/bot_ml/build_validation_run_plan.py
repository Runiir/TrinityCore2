...
RUNTIME_ASSET_PATH_ENVIRONMENT = tuple(
    (flag, name.upper())
    for name, flag in ARGUMENTS
    if name != "runtime_asset_map_id"
)


def _required_environment(name: str) -> str:
    return f"${{{name}:?set {name} to a canonical absolute path}}"


RUNTIME_ASSET_SHELL_EXPANSIONS = frozenset(
    _required_environment(name)
    for name in (
        *(name for _flag, name in RUNTIME_ASSET_PATH_ENVIRONMENT),
...
def shell_quote(value: str) -> str:
    if value in RUNTIME_ASSET_SHELL_EXPANSIONS:
        return f'"{value}"'
...
def live_validate_command(
    ...
    for flag, environment_name in RUNTIME_ASSET_PATH_ENVIRONMENT:
        command.extend([flag, _required_environment(environment_name)])
    scenario_map_id = int(scenario["map_id"]) if scenario.get("map_id") is not None else None
    command.extend(
        [
            "--runtime-asset-map-id",
            (
                str(scenario_map_id)
                if scenario_map_id is not None
                else _required_environment("RUNTIME_ASSET_MAP_ID")
            ),
        ]
    )
    return command
...
def build_plan(
    ...
    return {
        ...
        "runtime_asset_closure": {
            "binding_mode": "deferred_shell_environment",
            "required_path_environment": [
                name for _flag, name in RUNTIME_ASSET_PATH_ENVIRONMENT
            ],
            "map_id_source": "scenario.map_id_or_required_RUNTIME_ASSET_MAP_ID",
            "input_admission_owner": "tools.bot_ml.run_live_bot_validation",
        },
        "duration_policy": duration_policy,
        "reusable_session": {
            "enabled": reusable_session,
...
def write_shell_script(path: Path, plan: dict[str, Any]) -> None:
    lines = [
        ...
        "# Runtime asset paths are required environment bindings; each live validator admits them before runtime side effects.",
    ]
