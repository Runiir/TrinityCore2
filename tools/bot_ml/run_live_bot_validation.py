...
def write_validation_config(base_config: Path, output_dir: Path, pool_tag: str = "") -> Path:
    if not pool_tag:
        return base_config
    ...
    text += (
        "\n# Generated by tools.bot_ml.run_live_bot_validation for scenario-scoped bot pool selection.\n"
        f'BotWorld.PoolTagFilter = "{pool_tag.replace(chr(34), "")}"\n'
    )
...
def main() -> int:
    ...
    if args.transport == "process" and not args.input_log:
        effective_config = write_validation_config(args.config, args.output_dir, pool_tag_filter)
