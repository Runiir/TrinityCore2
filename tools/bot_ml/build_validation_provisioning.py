...
def build_character_insert_sql(
    ...
    if scenario_ids:
        ...
        selected = [row for row in all_scenarios if row["id"] in requested]
    ...
    lines = [
        "-- Generated by tools.bot_ml.build_validation_provisioning.",
        "-- Review before applying. This resets only selected validation characters and their owned items.",
    ...
    lines.append("DELETE i FROM `characters`.`character_inventory` i JOIN `characters`.`characters` c ON c.`guid` = i.`guid` "
                 f"WHERE c.`name` IN ({cleanup_names});")
    lines.append("DELETE i FROM `characters`.`item_instance` i JOIN `characters`.`characters` c ON c.`guid` = i.`owner_guid` "
                 f"WHERE c.`name` IN ({cleanup_names});")
    ...
    for scenario in all_scenarios:
        # Allocate from the complete frozen layout even for a selected subset.
        # Skipping other cohorts must not reuse their deterministic item IDs.
        if scenario_ids and scenario["id"] not in requested:
            item_guid += sum(len(bot.get("equipment", []))
                             + len(bot.get("consumables", config.get("default_consumables", [])))
                             for bot in scenario["bots"])
            continue
