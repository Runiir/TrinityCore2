...
def evaluate_calibration(
    ...
    if mode in {"single_target_300", "aoe_300"}:
        ...
        _check(checks, reasons, "overheal", float(metrics.get("overheal_ratio") or 1.0) <= float(mode_policy["maximum_overheal_ratio"]))
        ...
        _check(checks, reasons, "response_latency", float(metrics.get("response_latency_p95_ms") or 999999.0) <= float(mode_policy["maximum_response_latency_ms"]))
        ...
        _check(checks, reasons, "idle_under_demand", float(metrics.get("idle_ratio_under_demand") or 1.0) <= float(mode_policy["maximum_idle_ratio_under_demand"]))
        _check(checks, reasons, "cast_failure_ratio", float(metrics.get("cast_failure_ratio") or 1.0) <= float(mode_policy["maximum_cast_failure_ratio"]))
