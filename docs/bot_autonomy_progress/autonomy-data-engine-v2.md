...
Reproduce and push the updated route manifest, commit the scripted-transition/watchdog repair, rebuild exact `HEAD`, then run strict run 024. Require generation 1 to terminal only after four deaths plus Millhouse transition evidence; require steps 2-3 to observe their own generation-scoped Millhouse transitions; require the generation-3 `discovery_leg` contract, no Giant engagement/Quake during Corborus, eight-yard pull-anchor activation, and real boss kills. If health truly plateaus, require `semantic_progress_plateau_watchdog` before the 900-second emergency budget. Checkpoint and push every run before cleanup; do not start Phase 2 until the 10-clear Stonecore gate passes.
