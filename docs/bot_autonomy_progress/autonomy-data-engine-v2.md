...
Implement the completed-scripted-target hard mask and focus/pursuit release, rebuild exact `HEAD`, then run strict run 025. Require generation 1 to remain near its anchor and terminal after four deaths plus one Millhouse transition without enrolling downstream packs; require steps 2-3 to observe their own generation-scoped Millhouse transitions; require the generation-3 `discovery_leg` contract, no Giant engagement/Quake during Corborus, eight-yard pull-anchor activation, and real boss kills. Checkpoint and push every run before cleanup; do not start Phase 2 until the 10-clear Stonecore gate passes.
