...
Audit the unresolved generation-2 GUIDs from run 025 and repair only their actual liveness/selection cause; do not clear them for evade, path, LOS, unattackability, or failed resolution. Rebuild exact `HEAD`, then run strict run 026 and require generations 1-2 to terminal with matching death/transition evidence before reaching the generation-3 `discovery_leg`. Continue requiring no Giant engagement/Quake during Corborus, eight-yard pull-anchor activation, and real boss kills. Checkpoint and push every run before cleanup; do not start Phase 2 until the 10-clear Stonecore gate passes.
