...
- The next repair is a semantic hard mask and pursuit release for completed scripted GUIDs: reject them from target eligibility, clear only matching shared/per-bot focus and progress GUIDs, cancel only matching active pursuit movement, and preserve member/engagement/transition evidence plus every other live combat target. It must not call `AttackStop` or `CombatStop`.
