...
- `pixi run bot-lane-configs --lane 0 --dry-run`: passed. Lane 0 port allocation is world `18085`, auth `18086`, SOAP `13443`, RA `17878`; output root is `generated/bot_autonomy_lanes/lane_0`.
...
- Gear profile generation now prefers DB2/DBC plus hotfix rows, but falls back to a deterministic offline validation catalog when DB/DBC inputs are absent or MySQL is unreachable.
...
- Runtime lanes can use `pixi run bot-lane-configs --lane 0` to materialize isolated lane 0 configs under `generated/bot_autonomy_lanes/lane_0`.
