# Raid program error ledger: closed and historical entries

Moved from `docs/bot_raids/error_ledger.md` on 2026-09-23. Rows and sections are
verbatim; only relative link targets gained a `../` prefix so they still resolve
from this folder, and the details heading was renamed. Nothing here is current
instruction. Open items and the index of every ID are in
[the error ledger](../error_ledger.md); the tuning loop is in
...
| --- | --- | --- | --- |
| FLOW-009 | Completed-run reconciliation and compact entrypoints repaired | A coordinator marked a closed 300-second Blood run incomplete and claimed `scoring_ms` was missing despite the bound receipt containing `300000`. It rebuilt and repeated the unchanged setup, returning the same noncomparable reference. Broad reads caused three compactions. | Release/rework now checks exact operation receipts, including omitted receipts in the compact artifact directory. Use `workflow_step advance --recorded-source` to close historical evidence without current-source acceptance. Start/resume/advance default to compact output; `--full` is explicit. Never rerun combat to repair bookkeeping. |
...
| FLOW-004 | Plain-request entry and first diagnostic passed; full autonomous completion unproven | A fresh coordinator completed Balance assessment/publication, routed a new stat task, then ended its turn while all actor and encounter requirements remained open. A live-state test also required the old Balance setup edge forever. | `implement <boss> <mode> bots` resumes the whole saved objective. The CLI distinguishes coordinator/specialist ownership, exposes parent completion and the last assessment across routes. The primary executes successive stages; it does not require another user handoff. Regression fixtures cover progression instead of freezing current work. Evidence: `artifacts/cata_raid_program/plain_request_continuation_20260920.tar.gz.dvc`. |
| FLOW-003 | Earlier narrow closure retained; recurring defects superseded by FLOW-005 | New Python-only observer repair rebuilt native code under fast4 despite host12 guidance. DVC pointer source checks prompted pointer removal/restoration; latest pointer was hidden in `.git/info/exclude`. The original receipt-labelled independent review lacked separate reviewer evidence; a separate attributable review now exists. | The exact 300s Balance capture remains comparable at 31,877.553 DPS. `balance_setup` is narrowly published and accepted; performance, all actors, and raid clear remain open. The next routed edge is owner effective-stat application before rotation execution. Pointer visibility, remote DVC verification, and local cleanup are recorded; do not discard or rerun the valid capture merely for bookkeeping. |
| FLOW-002 | Workflow admission repaired; native performance unchanged/unmeasured | Builds at `8d43b7355f` and `8560ea0019` succeeded, but progress commits changed HEAD and dummy launch demanded exact HEAD equality. Balance catalog mismatch was discovered after compilation; a manually copied file hash also delayed admission. | Use existing build-control verifier in strict coordination mode; preserve original binary source and current clean coordinator identity. Build claims now run canonical reference checks first. Generate hashes with `workflow_build refs/snapshot`. Do not rebuild for committed progress alone. Evidence: `artifacts/cata_raid_program/workflow_build_repair_20260920.tar.gz.dvc`. Balance catalog and shared fixture gear mismatches remain separate setup blockers. |
| FLOW-001 | Fresh-entry read-only trial passed; no new native acceptance | Fresh "Implement Magmaw 10N Bots" thread used old `codex/magmaw-jev-canary`, selected a bounded native-script skill, copied its 25H example, and never loaded the saved mainline task. It stopped on dirty-source/configuration build admission; its final 17-pass summary omitted an earlier 29-fail/276-pass run. | `master` now lives in `trinity-cata`; AGENTS routes broad requests to the coordinator and exact requested mode. Continue the saved Balance setup diagnosis, retaining every actor requirement. Old unreviewed phase patch and unrelated edits are preserved in stash `61bd3108`; review separately, do not promote as a DPS fix. Evidence pointer: `artifacts/cata_raid_program/magmaw_entry_repair_20260920.tar.gz.dvc`. |
| CAL-001 | Native concurrent dummy lifecycle repaired and live exercised on `50ec676cdb` | All five exact 300-second windows completed in distinct native phases; addressed cleanup preserved peer clock/damage and servers exited 0. Initial failure was first-heartbeat area initialization clearing direct phases, with stale provisioning observations hiding it. Initial evidence: `dummy_b8e9803721_initial_20260919.tar.gz.dvc`. | Preserve area-before-phase ordering and fresh native phase observations. Current setup mismatches are separate from isolation and measured DPS. |
...
| DPS-057 | Native Combustion accepted for30007 ona3a; overall Fire open | Restored Ignite/LB input mask and base-period rate; both Fire actors remain separately reviewed. Fire30007 rose19.85%,30006 fell9.19%. | 30007 child83853 landed16ticks/549,730. Combined Fire damage+5.60%; baiter proc/periodic losses with shorter casting gaps show no new gate defect. Exact pre-cast sum/crit join remains missing; no overall DPS acceptance. |
| DPS-049 | Admission accepted on63d; native behavior repaired under DPS-053 ondb67 | Summon18540 creates11859 for45s alongside primary417. | Preserve admission and normal lifetime. Overall Affliction remains open. |
| DPS-053 | Native caster repair accepted ondb67 | Nine Doom Bolts/102,447 damage, zero guardian melee, Felhunter preserved. Owner603 body targeting works; later603 parasite placement removes useful body target. | Preserve Bane-aware native AI and shared selection. Numeric owner-SP parity remains unproved; route Bane placement to DPS-056. Initial arbitrary owner-target design was corrected before deployment. |
| DPS-048 | Survival AoE exercised on d495; Horn healthy | Two legal Multi-Shots/Spread, no unexplained in-window Cobra cancellation; exact movement receipt owns the Mangle staging interruption. Wolf friendly damage is post-death only. | Preserve shot policy and mandatory movement. Route post-clear pet combat separately; model assignment labels do not override completed native duties. |
| DPS-050 | Missing AP hypothesis rejected; historical MM overstacking proven | Blood53138 already provides20% melee AP. Historical18867*1.2*1.2=27168.48 exposes duplicate Trueshot; Horn/Shout provide separate flat stats. | Do not add Might as compensation. Route native same-effect group1126 omission to DPS-058; preserve current single AP category. Equipment/Heart Strike remain separate. |
| DPS-051 | Native admission accepted on779 | 33831 submitted/finished; three owned1964 guardians,34 landed melee hits and83,995 damage. | Preserve guardian authority and true AoE gates. Remaining Balance cadence/assignment differences are separate. |
| DPS-052 | Both actors load40; exact35–40yd branch unexercised | Only action2158 changed35 to40; every other action/profile field unchanged. Native build/canary completed ona3a. | Both actors expose28samples at40; one real cast at8.033yd. Retain deterministic37.7469 counterexample; no unchanged run merely to force coverage. |
| DPS-045 | Survival swap native accepted; broader role gaps remain | Current05f Hunter37,131DPS and raid216,509DPS; same Blood/three-healer/six-DPS roster. WCL references remain qualified comparisons, not matched floors. Missing Doomguard, misclassified Force of Nature, long parasite DoTs and absent permanent role enchants remain independently documented. | Select one retained production-backed class/role edge for the next repair. Keep Blood/Balance declines visible; verify exact stats/cadence before coefficient changes. See magmaw_dps_baseline_20260913.md. |
| DPS-046 | Accepted native Survival shot/ST admission repair | Source699 native head Cobra/core shots land; former head range/count rejects absent. Survival14603.545→37019.760DPS. Eight production-gate tests and independent review passed; native updater/hash/rows verified. | Preserve accepted shot caps40/Kill45 and coreST density correction. Overall raid191627.988DPS/six Infection hits fails safety/performance; route invalid remote trap toDPS-047 and caster head movement/range loss separately. |
| DPS-047 | Accepted narrow native trap removal on05f | Zero trap13813 attempts/reconciliation and zero Infection hits; head Cobra/core shots preserved. Raid130.217s/216,508.774DPS, Hunter37,131.143DPS. One trash casualty recovered, no boss deaths. | Preserve the fix. Multi-Shot loaded/unchanged but native use unobserved this run; no eligible bypass proven. Head ends in death, so body-return behavior unexercised. Overall single-run throughput recovered; class/WCL parity remains unaccepted. |
| DPS-043 | Independently reviewed and native accepted on ee0504 | Fixed baiters bypassed filtered opportunities for raw NearestParasite; both Multi-Shots on26 failed LOS. | 13 focused tests pass, including actual binding and preserved escape ownership. On ee0504 all13 Mage/15 Hunter parasite binds observe LOS true; Hunter has two successful Multi-Shots/11 landed effects and no native rejections. Mage Blast Wave hits8 parasites and Flamestrike lands17 events. Zero Infection or deaths. Exact live blocked-nearer alternative is unobserved. Raid132.205s/213,253.077DPS; Hunter31,513 and bait Mage27,936 remain below26, so all-class optimization is unaccepted. |
| DPS-042 | Shared range repair independently reviewed and native accepted on26; exact Balance under5 native-success coverage absent | On433, core Balance spells reject at4.650yd despite native/configured minimum0. Initial2af patch missed a handler returning5yd movement for larger actual minima. | Unified native/configured minimums across producer, consumers and rejection recovery.16 tests pass; old433 fails the zero-min case and2af fails the eight-yard movement envelope. Hunter receipt307 on26 moves11.163→25.323yd and Auto/Steady land. Balance recovers to25,641DPS, but exact post-exit under5 state does not recur; pincer attempts at0.765 reach native and fail176. No unchanged retry. Raid133.998s/210,399.581DPS/10,868.625HPS; class performance remains mixed. |
| DPS-041 | Independent review and native ordering repair accepted on433; throughput unresolved | Mandatory Hunter setup bypassed low-priority Mark ranking on changing parasites. | Twelve focused tests passed. Latest ordinary add setup Marks13→0, ranked Marks0→6, boss/head setup preserved, Multi-Shot finished and landed. Hunter DPS35,127→33,245 and eleven late Sting LOS failures prevent performance acceptance. Retain the repair while diagnosing distinct range/LOS gaps. |
| OBS-011 | Timeline projection repaired on067; existing native records reused | Actual effective/combat stats were retained in raw captures but omitted from the primary timeline. | Emit timed diagnosis_stats_observation with actor/attempt identity and original observation clock; never invent missing clocks.39 tests pass;433 replay retains700 snapshots with unchanged accounting/phases. Full-run HTML grows6772210→7444881 bytes; no new server logger or capture overhead. |
| MOV-001 | Native hazard traversal accepted on779 | Receipt373 used native_same_surface_hazard_escape, moved4.406to0.176yd from selected endpoint and completed with ordinary work resumed. | Preserve native path/executor guards; zero Infection accompanies actual exercised traversal. |
...
| DPS-063 | Moving-cast facing accepted on d495 | Five moving Scorch preparations succeeded, four finished moving, zero result136 across both Fire actors. A dead-parasite result13 remains distinct. | Preserve native arc checks and movement. This repairs the bounded facing defect, not overall Fire throughput. |
...
| DPS-040 | Rejected directional mobility changed facing; repair accepted | Source b521 Fire30006 had 10 failed/0 successful exposed-head Fireballs alongside 62 failed Blinks. Native executor changed facing before cooldown/GCD rejection. | Readiness now precedes facing/native submission; other native rejections restore facing. Six focused tests, independent review and native build passed. Source c570 had 17 successful/0 failed head Fireballs, all landed for 993,679 damage, with typed cooldown rejection before native Blink. Clear: 147.272s, 191,961.907 DPS, above the 189,706.381 matched baseline. No blanket class acceptance; low-value Hunter Mark remains separate. |
| REC-003 | Finalized ghost spline waited for 30s timeout; repair accepted | Source 3329 actor30006 stopped short of the entrance with native idle/empty active slot/finalized spline. Retained path evidence kept it waiting; total recovery was 78.198s. | Retry the exact finished native path once; retain 30s fallback for active paths. Seventeen tests, independent review and build passed. Source b521 actor30005 repathed 1.499s after physical progress, continued on a new spline, and returned in 53.047s with retry count 1. The exact finalized pre-repath frame fell between diagnosis samples. Source c570 returns were 41.846-46.455s. RecentDeathCount reset across focus cleanup remains a separate bookkeeping issue. |
| REC-002 | Partial-trash release wait repaired | Source 0f0 actor30007 waited 78.926s before release while the hostile/reset gate blocked a partial casualty; native release-to-alive took 43.567s. | Permit attributable partial-trash release after combat-res admission; retain boss/full-wipe/native entrance checks. Source c570 had eight death episodes, releases within 1.514s and returns in 41.846-46.455s. No native speed changes. A universal 30-40s return and full-wipe live coverage are not established. |
| PULL-001 | Hunter Drudge opener verified; route speed not accepted | The current opener uses SplitSeedRosterSlots.front()=8, so Warlock30008 submitted the pull. Hunter30009 is present. Source0f0 also waits roughly25s at Chainwielder safe-phase after staging; whether an earlier safe pull is available remains unproven. | Prefer the frozen roster's Hunter for the Drudge opener, retain the declared safe pull position and later seed assignments, require native Misdirection to the assigned tank. Preserve staging/native paths. Do not remove the Chainwielder safety wait without proving an earlier safe native path. |
...
## Closed error details

### FLOW-005: Moonkin handler observation and workflow failures

- The separate review found the prior trace was logged to the disabled `spells`
  category. The bounded patch moved only the diagnostic markers to the enabled
  `server` logger; the native arithmetic, exclusivity checks and direct-cast
  modifier calls are unchanged.
- The canonical process probe started with the complete six-root runtime-asset
  closure, reset the candidate pool, and shut down cleanly. Actor 1310 recorded
  owner apply `0.827578366 -> 0.788169861` and owner remove
  `0.788169861 -> 0.827578306`, plus the expected spell-group-5 exclusive skips.
- This proves handler execution on the observed startup path. It does not prove
  scoring-start effective-stat parity, DPS, actor acceptance or a Magmaw clear.
  The historical next step was to inspect the downstream transition after
  handler application. Later reference review superseded that missing-haste
  hypothesis: WoWSims already includes the 5% form multiplier, so the correct
  native expectation is 1.26876, matching observed 1.269. Preserve the comparator
  correction; do not repeat the probe to chase the old 1.3322 expectation.
  Actor performance and encounter requirements remain open under FLOW-006.
- Raw capture: `artifacts/cata_raid_program/magmaw_balance_effective_stat_application_04_short_probe_20260920.tar.gz.dvc`.
