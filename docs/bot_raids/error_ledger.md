...
Updated 2026-09-10. This is the canonical index of known blockers and rejected
...
| --- | --- | --- | --- |
| ENC-002 | Contract installed on 41266; native swap rejected | Initial ff34 contract omission is repaired. On 41266 adaptive ownership skips both callers of the swap handler; the generic candidate additionally conflicts with admitted movement. No route4 taunt submission or outcome. The inner-gate fixture missed actual dispatch. | Expose the existing swap as a narrow typed action without reopening generic boss movement. Regression must cross production submission and kernel resolution with adaptive ownership and concurrent movement. Preserve normal two-tank alternation. |
...
| DPS-036 | Fire moving Scorch submissions fail native completion on 41266 | Bait30006 has50 attempts,47 accepted submissions,47 unsuccessful native finishes and zero Scorch damage. Hook30007 has one successful native finish and39,465 damage. | Hazard executor cancellation before active-path retention is proven. Independently reviewed repair checks both Spell::CheckMovement and Unit::IsMovementPreventedByCasting; first review caught their triggered-channel disagreement. Repeated covered-cast regression fails old source. Native completion remains to validate; no coefficient change. |
