...
Updated 2026-09-20. This is the canonical index of known blockers and rejected
...
| --- | --- | --- | --- |
| FLOW-005 | Logger visibility repair reviewed, built and live-observed; downstream native cause and performance remain open | Agent bundled stat reporting and aura hypotheses, double-counted shared haste, used source-shape tests as native proof, and authored its own independent-review adapters. Repeated full scoring runs left speed1.269 unchanged. The reviewed server-logger repair now exposes the 24907 handler: owner apply and remove both execute, while the 30-second probe never enters scoring. Build attempts repeated wrong argv, dirty claims and stale configure lineage; DVC publication prompted revert/hide churn. | Preserve the proven handler boundary and route the downstream effective-stat transition. Do not change arithmetic or run another full scoring window until a trace-backed repair or throughput question warrants it. Use policy-derived `workflow_build run`, separate reviewer session/report, and cross-unit attempt history. Keep all actor baselines and requirements visible. Evidence: `artifacts/cata_raid_program/magmaw_balance_effective_stat_application_04_run_20260920.json`, `artifacts/cata_raid_program/magmaw_balance_effective_stat_application_04_assessment_20260920.json`, and `artifacts/cata_raid_program/magmaw_balance_effective_stat_application_04_short_probe_20260920.tar.gz.dvc`. |
...
### FLOW-005: Moonkin handler observation and remaining effective-stat gap
...
- This proves handler execution on the observed startup path. It does not prove
  ...
  The next unit must inspect the downstream transition after handler
  application. The exact actor and encounter requirements remain open.
