...
| --- | --- | --- | --- |
| DPS-042 | Native-range repair independently approved; live validation pending | On433, Balance remains at4.650yd after pincer duty. Core spells report ranged_range_required while movement says profile_min_range_satisfied at configured0. Native caster minimums are0; Hunter ranged flags impose a real deadzone. Balance DPS27,512→17,670; matched head damage1,278,205→543,729. | Shared native/configured minimum helper replaces three artificial gates. Fifteen focused tests pass independently; frozen433 fails the recorded-point assertion. Build and run one bounded native validation, preserving Hunter deadzone and movement ownership. Earlier Eclipse carryover is unknown; do not revert Hunter or tune coefficients without evidence. |
