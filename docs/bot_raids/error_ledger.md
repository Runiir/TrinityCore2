...
| PET-001 | Elemental classification repair independently approved; live acceptance pending | First friendly effect is Fire Shield area damage to a same-raid wolf; wolf explicit retaliation follows, then elemental melee. Through death: elemental→wolf159,090 and wolf→elemental12,260. Runtime control flags were not logged; source exposes the skipped initialization. | Initialize player control before victim/area search and preserve native target-validity rules, including lawful hostility. Do not introduce an unconditional raid-membership ban. Require legal-enemy positive fixture and zero friendly-pet damage in the next live run. |
