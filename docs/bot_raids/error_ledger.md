...
| --- | --- | --- | --- |
| CAL-001 | Concurrent dummy isolation failed before scoring on `b8e9803721`; lifecycle repair pending validation | Both Survival and Fire returned `calibration_isolated_target_lost` at zero scored seconds. Spawn placement leaves zone/area initialization for the first Player heartbeat; `OnAreaChange` clears the directly assigned phase. Cached provisioning observations still reported a phase match. Closed evidence: `artifacts/cata_raid_program/dummy_b8e9803721_initial_20260919.tar.gz.dvc`, remote MD5 `7e090fb643f28c4d43d0f90d054fe3a6`, all72members verified. | Initialize native zone/area before the fixture phase, read current phase observations, and prove the concurrent pair through scoring and addressed cleanup. Do not reapply phases each tick or interpret this setup failure as low class DPS. |
| REF-003 | Retained reference inventory corrected; current class parity remains open | A reviewer labeled legacy embedded DPS as current promotion and compared different gear-hash projections. Historical optimization flags allow 85% of reference. Neither establishes current class parity. | Use raid_workloop and the verified pet_identity_v4 request cohort. Reuse existing 300s Fire/Affliction/Elemental evidence; verify historical Balance conditions; fill missing exact-300 Survival coverage. Compare semantic setup fields before hashes. |
