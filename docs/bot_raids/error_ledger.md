...
| DPS-040 | Unavailable directional mobility changes facing; repair reviewed | b521 Fire30006 repeatedly submits rejected Blink every500ms during a27.039s direct-hit gap with failed Fireballs. Native executor turns toward escape before checking native cooldown/GCD. The orientation side effect is proven; its share of total DPS loss is not. | Check native readiness before facing/cast; restore prior facing on later native rejection. Preserve ready emergency casts and typed fallback.6 behavioral/integration tests and independent review pass; build/live pending. Separately, Hunter spends4GCDs marking short-lived parasites; next policy finding retained. |
| REC-003 | Finalized native ghost spline retained until30s retry; repair reviewed | Source3329 actor30006 receipt277 had native idle/empty active slot and finalized spline outside entrance; position stopped, but matching retained path caused waiting until1789239371904. Total return78.198s. | Repath that exact finished native state once through existing executor/retry token. Preserve30s fallback for active paths.17 focused tests, independent review and native build pass. Sourceb521 actor30005 repathed1.499s after physical progress, continued on a new native spline and returned in53.047s; retry stayed at1. Exact idle/finalized pre-repath frame fell between diagnosis samples. Five casualties recovered and route cleared; overall DPS acceptance remains separate. RecentDeathCount reset during trash-focus cleanup also restarts episode bookkeeping but did not cause this stall; separate work. |
| REC-002 | Partial trash release wait repaired; native runback tail remains | On 0f0 actor30007 died at1789232969697 and released at1789233048623. Eighteen retained waits cited hostile activity and two native reset; actual release-to-alive took43.567s. UpdateDeath applies NativeFullWipeOnly reset gating to partial trash deaths. | Permit attributable partial trash release after combat-res admission while preserving boss/full-wipe/native entrance checks. Do not change native speed or promise a universal30-40s path. Fixtures, independent review and native build passed. Source3329 releases five trash casualties in0-1.499s; four return42.541-44.815s, one78.198s after a native path stalls until the30s retry. Do not claim the full30-40s target is met. |
