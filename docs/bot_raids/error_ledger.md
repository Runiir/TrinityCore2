...
| --- | --- | --- | --- |
| FLOW-005 | Workflow retry/review defects repaired; native cause and performance remain open | Agent bundled stat reporting and aura hypotheses, double-counted shared haste, used source-shape tests as native proof, and authored its own independent-review adapters. Repeated full scoring runs left speed1.269 unchanged; new trace logs to disabled spells INFO. Build attempts repeated wrong argv, dirty claims and stale configure lineage; DVC publication prompted revert/hide churn. | Use policy-derived `workflow_build run`, separate reviewer session/report, and cross-unit attempt history. New output-only DVC metadata no longer invalidates graph source binding; selected inputs remain protected. Review prior speculative gameplay patches and prove observation visibility before a short lifecycle probe. Keep all actor baselines and requirements visible. Evidence: `artifacts/cata_raid_program/workflow_loop_repair_20260920.tar.gz.dvc`. |
