...
| REC-003 | Finalized native ghost spline retained until30s retry; repair reviewed | Source3329 actor30006 receipt277 had native idle/empty active slot and finalized spline outside entrance; position stopped, but matching retained path caused waiting until1789239371904. Total return78.198s. | Repath that exact finished native state once through existing executor/retry token. Preserve30s fallback for active paths.17 focused tests and independent review pass; native build/live pending. RecentDeathCount reset during trash-focus cleanup also restarts episode bookkeeping but did not cause this stall; separate work. |
