...
| --- | --- | --- | --- |
| ENC-002 | Magmaw tank-swap contract independently approved; live pending | Native 78199 applies on both DK Mangles; second fatal tick45,452 kills DK. No tank taunt is submitted because the route mechanic contract omits the existing debuff swap trigger. | Add debuff_stacks/78199/1 to the two Magmaw contracts and exercise the actual two-tank policy. Keep a152 rejection of premature DK reclaim; proper Paladin coverage is required. |
| DPS-035 | Fire corrective FLOAT migration independently approved; live pending | ff34 has only Scorch row55. MySQL FLOAT max_mana_pct=0.40 evaluates false for stored0.4000000059604645; migration07 inserted no moving row. SQLite fixture missed storage precision. | Preserve applied07; corrective08 uses a storage-safe predicate and float32 counterexample. Require actual database row readback after native updates and before another encounter. Missing row is setup failure, not a new casting defect. |
...
| DPS-033 | Fire moving Scorch unexercised on ff34; setup blocker DPS-035 | Fire bait has 42.500 seconds of moving Fireball rejection with Scorch mana-gated; 38.700 seconds have zero valid profile actions. | Add scoped moving Scorch with native movement capability and resource checks; preserve stationary low-mana row and higher valid priorities. |
