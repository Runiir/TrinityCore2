...
| --- | --- | --- | --- |
| DPS-041 | Bounded Hunter repair and fixture ready for independent review | In89cef, forced Hunter's Mark setup submits three marks on changing parasites before the first active damage shot;31 marks total. It bypasses the existing bucket6 profile ranking intended to prevent that delay. | Keep pre-pull and native boss setup; ordinary combat targets use existing ranking. Native boss methods/type flags are authoritative, not rank alone. Twelve fixtures pass and fail on the old producer. Not included in MOV-001 live sourceff226. |
