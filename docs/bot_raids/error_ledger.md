...
| --- | --- | --- | --- |
| DPS-042 | Zero-minimum canary closed; propagation correction approved, native validation pending | On433, Balance remains at4.650yd after pincer duty. Core spells report ranged_range_required while movement says profile_min_range_satisfied at configured0. Native caster minimums are0; Hunter ranged flags impose a real deadzone. Balance DPS27,512→17,670; matched head damage1,278,205→543,729. | 2af clears131.609s/214,218.807DPS with Balance29,498DPS, but introduced a missed pre-rejection consumer that still propagates only5yd for larger actual minima. The corrective patch propagates the effective minimum; its producer→rejected-handler→movement fixture fails on2af and16 focused tests pass independently. Shared native/configured minimum helper replaces three artificial gates. Fifteen focused tests pass independently; frozen433 fails the recorded-point assertion. Build and run one bounded native validation, preserving Hunter deadzone and movement ownership. Earlier Eclipse carryover is unknown; do not revert Hunter or tune coefficients without evidence. |
