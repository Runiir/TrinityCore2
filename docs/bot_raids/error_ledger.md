...
| --- | --- | --- | --- |
| DPS-030 | Affliction combat potion repair independently approved; live pending | Actor30008 pre-pots and reaches execute health, but its raid profile contains no79476 action despite a reserved second58091 item and the exact APL's second potion. | Add the ordinary item candidate with observable execute-health timing. First verify native LastPotionId lifecycle and shared reservation; never clear lockout or install an aura to force use. |
...
| DPS-027 | Marksmanship aura and filler repair independently approved; live pending | Actor30009 checks53221 for Steady Shot maintenance; native paired Steady Shots apply haste53220. Required53221 is missing in107 stationary masks. | Scoped aura53220 gates plus exact-APL final Steady filler preserve focus generation; seven focused tests and independent review pass. Review caught MySQL comment syntax missed by SQLite; corrected with a compatibility check. Validate native haste cadence on the combined run. |
| DPS-028 | Elemental movement cooldown repair independently approved; live pending | Actor30010 lacks79206 in learned spells and action profile; four movement-only masks reject otherwise-ready Lava Burst solely for movement. | Provision Spiritwalker's Grace through the canonical spec catalog and admit its native action for a blocked moving Lava Burst opportunity. Glyph101052 affects Lightning Bolt only; do not change glyph or spell coefficients. |
| DPS-029 | Affliction Shadowflame cadence unresolved | Actor30008 has zero Shadowflame submissions in both retained Magmaw attempts; current valid masks can be passive previews. | Inspect authoritative execution masks after OBS-004. Do not equate preview selection with a lost native cast or change priorities without that join. |
...
| OBS-004 | Preview publication repair independently approved; combined build pending | Lower-priority range observation overwrites all five last-combat diagnostic maps, making an unsubmitted preview look like the execution choice. | Explicit non-publishing mode at two passive callers;16 focused tests pass. Preserve execution publication and returned preview action. |
| OBS-005 | False fallback reason repair independently approved; combined build pending | Spell0 successful melee fallback has no SpellInfo; false CooldownReady/HasPower fields synthesize cooldown/no_power reasons. | Infer those reasons only with spell metadata. Four focused tests preserve explicit overrides and real spell failures. |
