...
| --- | --- | --- | --- |
| DPS-042 | Proven native-range admission mismatch; implementation in progress | On433, Balance remains at4.650yd after pincer duty. Core spells report ranged_range_required while movement says profile_min_range_satisfied at configured0. Native caster minimums are0; Hunter ranged flags impose a real deadzone. Balance DPS27,512→17,670; matched head damage1,278,205→543,729. | Unify producer/consumer admission on native and configured minimums, preserve Hunter deadzone and movement ownership, test the recorded point, obtain independent review and bounded native validation. Earlier Eclipse carryover is unknown; do not revert Hunter or tune coefficients without evidence. |
