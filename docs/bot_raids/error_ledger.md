...
| --- | --- | --- | --- |
| ENC-003 | Shared optional parasite target admission deadlock proven on6882 | Fire hook has244 damage in final57.689s with valid parasite selected;38/46 samples defer combat. Support target binding lacks LOS/executable-range facts, but support movement forbids LOS/max-range reposition. Hunter has23 native Serpent Sting LOS failures; not every interval has the same contract role. | Admit optional support only with actor-target spatial legality. Preserve personal threats, bait and hazard ownership. Pure strategy facts currently lack LOS; supply observed facts or validate in the native target adapter. No invented legal fallback or generic movement relaxation. |
