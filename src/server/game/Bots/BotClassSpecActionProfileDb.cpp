    ...
    for (std::string const& key : snapshot.Order)
    {
        ...
        for (BotActionProfileSpell const& spell : profile.Spells)
            payload << spell.SortOrder << '|' << spell.SpellId << '|'
                    ...
                    << spell.MinOwnedTargetAuraRemainingMs << '|' << spell.MaxHostileTargetHealthPct << '|'
                    << uint32(spell.MaxReadyRunes) << '\n';
    }
    return payload.str();
}
...
std::shared_ptr<DbRotationSnapshot> LoadDbSnapshot(std::string& failureReason)
{
    ...
    QueryResult result = WorldDatabase.Query(
        ...
        "a.max_self_aura_charges, a.min_owned_target_aura_remaining_ms, a.max_hostile_target_health_pct, "
        "a.max_ready_runes "
    ...
    do
    {
        ...
        spell.MaxHostileTargetHealthPct = fields[80].GetFloat();
        spell.MaxReadyRunes = fields[81].GetUInt8();
        ...
        if (!ValidHostileTargetHealthRange(spell))
        ...
        if (spell.MinReadyRunes > spell.MaxReadyRunes && spell.MaxReadyRunes)
            invalidReasons.insert("invalid_ready_rune_range_" + key + "_" + std::to_string(spell.SortOrder));
...
std::string BotClassSpecActionProfileStore::DbProfileDumpJson(uint8 classId, std::string const& specTag, std::string const& role)
{
    ...
    for (BotActionProfileSpell const& spell : profile.Spells)
    {
        ...
        json << "{\"sort_order\":" << spell.SortOrder
             ...
             << ",\"max_ready_runes\":" << uint32(spell.MaxReadyRunes)
             ...
             << ",\"max_hostile_target_health_pct\":" << spell.MaxHostileTargetHealthPct
