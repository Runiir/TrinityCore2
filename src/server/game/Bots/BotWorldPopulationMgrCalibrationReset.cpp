...
// Initial resource contract observations, including
// InitialResourceSourceContract and PreScoreGlobalCooldownClear, live in the
// dedicated resources-reset translation unit. The scored-window reset below
// consumes that state without changing the calibration boundary.
...
void BotWorldPopulationMgr::ResetCalibrationScoredWindow()
{
        ...
        && Cohort().CalibrationMode == "single_target_300")
    {
        ...
        for (WorldBotState const& state : Party().CalibrationBots)
        {
            ...
            CalibrationMetrics const& metrics = metricsItr->second;
            bool const precombatConsumablesComplete =
                metrics.PreScoreCooldownResetComplete
            ...
            consumablesSettled = consumablesSettled
                && precombatConsumablesComplete
                && metrics.LastPreScoreConsumableFinishedUpdateOrdinal
                && metrics.WarmupUpdateOrdinal
                    > metrics.LastPreScoreConsumableFinishedUpdateOrdinal;
        }
        // A completion callback can precede the native owner-stat update for
        // its item aura. Keep t=0 closed until a later warmup update has run.
        if (!consumablesSettled)
            return;

        // Recalculate the existing live pet only at the final pre-score
