...
def _staging_fixture(tmp_path: Path) -> dict[str, object]:
    root = tmp_path / "source"
    route = root / "dataset/validation_scenarios/validation_routes.jsonl"
    route.parent.mkdir(parents=True)
    route.write_bytes(b'{"scenario_id":"generic","routes":[]}\n')
    member_md5 = hashlib.md5(route.read_bytes(), usedforsecurity=False).hexdigest()
    manifest = json.dumps([
        {"md5": member_md5, "relpath": "validation_routes.jsonl"},
    ], sort_keys=True, separators=(",", ":")).encode()
    directory_md5 = hashlib.md5(manifest, usedforsecurity=False).hexdigest() + ".dir"
    cache = root / ".dvc/cache/files/md5" / directory_md5[:2] / directory_md5[2:]
    ...
    external.mkdir()
    return {
        "root": root, "route": route, "external": external,
...
def test_external_route_staging_authenticates_dvc_and_writes_receipt(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch,
) -> None:
    fixture = _staging_fixture(tmp_path)
    create_calls = 0
    ...
    receipt = prestart_bundle.stage_canonical_route(
        worktree=fixture["root"], source_route=fixture["route"],
        expected_sha256=fixture["sha256"], external_run_root=fixture["external"],
    )
    ...
    assert receipt["source_sha256"] == receipt["staged_sha256"] == fixture["sha256"]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("dirty_head", "source_worktree_dirty"),
...
def test_external_route_staging_fails_closed_before_bundle_create(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, mutation: str, reason: str,
) -> None:
    fixture = _staging_fixture(tmp_path)
    route, root = fixture["route"], fixture["root"]
    expected, external = fixture["sha256"], fixture["external"]
    if mutation == "missing":
        route = root / "dataset/validation_scenarios/missing.jsonl"
    elif mutation == "source_drift":
        route.write_bytes(b"drift\n")
    elif mutation == "caller_hash_drift":
        expected = "0" * 64
    elif mutation == "dirty_head":
        ...
        monkeypatch.setattr(
            prestart_bundle, "_copy_exact",
            lambda _source, destination: destination.write_bytes(b"drift"),
        )
    elif mutation == "destination_conflict":
        (external / f"{route.stem}-{expected}{route.suffix}").write_bytes(b"occupied")
    ...
    with pytest.raises(BundleError, match=reason):
        prestart_bundle.stage_canonical_route(
            worktree=root, source_route=route, expected_sha256=expected,
            external_run_root=external,
        )
    assert create_calls == 0
