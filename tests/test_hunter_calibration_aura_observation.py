...
struct Player { AuraApplication* howl=nullptr; AuraApplication* mastery=nullptr; Pet pet;
 AuraApplication* GetAuraApplication(unsigned id){return id==24604 ? howl : mastery;}
...
int main(){
 ...
 howlApplication.mask=2; M::ObserveCalibrationOwnerAuras(metrics,&owner,200);
 howl.caster={11}; howlApplication.mask=1; effect.amount=6;
 M::ObserveCalibrationOwnerAuras(metrics,&owner,300);
 howl.caster={0}; howl.effect=nullptr; M::ObserveCalibrationOwnerAuras(metrics,&owner,400);
 ...
 assert(M::statsWalks==1 && metrics.OwnerAuraObservations.size()==2);
 std::ostringstream full; full<<"{\\\"fixture\\\":true"; M::AppendCalibrationOwnerAurasJson(full,&metrics);full<<"}";
 assert(full.str().size()<json.str().size()+150);
...
''')
    ...
    howl, mastery = observed['auras']
    assert [row['spell_id'] for row in observed['auras']] == [24604, 76659]
