...
def inputs():
    ...
    client = {'rows': {'SpellEffect': [dict(SpellID='78359', EffectIndex='0',
        ...
        EffectBasePoints='30624', EffectDieSides='8751', EffectRealPointsPerLevel='0',
        EffectPointsPerResource='0', BonusCoefficientFromAP='0', Coefficient='0',
        ResourceCoefficient='0', Variance='0')]}}
...
@pytest.mark.parametrize('field', ['EffectRealPointsPerLevel', 'EffectPointsPerResource',
                                 'BonusCoefficientFromAP', 'Coefficient',
                                 'ResourceCoefficient', 'Variance'])
def test_unsupported_or_missing_formula_is_rejected(field):
    client, observations = inputs()
    effect = client['rows']['SpellEffect'][0]
    effect[field] = '0.1'
    with pytest.raises(ValueError, match='unsupported damage formula'):
        compare(client, observations, 0)
    del effect[field]
    with pytest.raises(ValueError, match='unsupported damage formula'):
        compare(client, observations, 0)
...
def test_empty_selection_is_not_compatible_evidence():
    client, observations = inputs()
