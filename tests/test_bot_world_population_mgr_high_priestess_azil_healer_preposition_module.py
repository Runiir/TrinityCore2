...
def test_azil_healer_add_wave_preposition_boundary_is_registered_and_ordered():
    ...
    assert "HealerAddWavePrepositionRequest" in world

    guard = world.index("ValidationRouteAddTargetEntries.empty()")
    dispatch = world.index("TryHealerAddWavePreposition(", guard)
    generic_adds = world.index("Unit* add = nullptr;", dispatch)
    assert guard < dispatch < generic_adds
