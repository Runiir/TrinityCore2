...
Keep as little data on local disk as possible; offload it to DVC.
...
No model is enforced for delegated agents: start subagents without a model
override and let the tool use its default. Keep implementer and reviewer in
separate sessions. When a delegated task stalls, narrow it or improve its
evidence. Jev and Laya are removed from the workflow (2026-09-23): no step calls them; their code stays on disk. Historical receipts retain their
actual model identities; they impose no model requirement on new work.
